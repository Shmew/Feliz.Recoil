...
module Bindings =
    ...
    type Recoil =
        ...
        static member useSetRecoilState<'T> (recoilState: RecoilValue<'T,WriteOnly>) : U2<'T -> unit,('T -> 'T) -> unit> = import "useSetRecoilState" "recoil"
        static member useResetRecoilState<'T> (recoilState: RecoilValue<'T,ReadWrite>) : unit -> unit = import "useResetRecoilState" "recoil"
        static member useResetRecoilState<'T> (recoilState: RecoilValue<'T,WriteOnly>) : unit -> unit = import "useResetRecoilState" "recoil"
        static member useRecoilCallback<'T> (f: obj, ?deps: ResizeArray<obj>) : 'T = import "useRecoilCallback" "recoil"
        static member useRecoilTransactionObserver (callback: obj -> unit) : unit = import "useRecoilTransactionObserver_UNSTABLE" "recoil"
        static member useGotoRecoilSnapshot () : (Snapshot -> unit) = import "useGotoRecoilSnapshot" "recoil"
        static member useRecoilSnapshot () : Snapshot = import "useRecoilSnapshot" "recoil"
        static member useTransactionObservation<'Values,'Metadata> (callback: TransactionObservation<'Values,'Metadata> -> unit) : unit = import "useRecoilTransactionObserver_UNSTABLE" "recoil"
